...
ggplot(dat_long, aes(x = time, y = bprs, color = sex)) + 
  ...
  theme_minimal() + 
  labs(
    title = 'Individual BPRS Score as Function of Time and Sex',
    x = 'Year',
    y = 'BPRS Score'
  )
...
ggplot(dat_long, aes(x = time, y = bprs, color = sex, group = patid)) + 
  ...
  theme_minimal() + 
  labs(
    title = 'Individual BPRS Score Profiles Over Time by Sex',
    x = 'Year',
    y = 'BPRS Score'
  )
...
ggplot(dat_long, aes(x = time, y = bprs, color = sex, group = sex)) + 
  ...
  theme_minimal() + 
  labs(
    title = 'Average BPRS Score as Function of Time and Sex',
    x = 'Year',
    y = 'BPRS Score'
  )
...
ggplot(dat_long, aes(x = time, y = bprs, color = age)) + 
  ...
  theme_minimal() + 
  labs(
    title = 'BPRS Score as Function of Time and Age',
    x = 'Year',
    y = 'BPRS Score'
  )
...
ggplot(dat_long_sample, aes(x = time, y = bprs, group = patid, color = age)) +
  ...
  theme_minimal() +
  labs(
    title = "Individual BPRS Trajectories (Sample of 400 Patients)",
    x = "Year",
    y = "BPRS Score"
  )
...
ggplot(dat_long, aes(x = time, y = bprs, color = wzc)) + 
  ...
  theme_minimal() + 
  labs(
    title = 'BPRS Score as Function of Time and WZC',
    x = 'Year',
    y = 'BPRS Score'
  )
...
ggplot(dat_long, aes(x = time, y = bprs, color = wzc, group = wzc)) + 
  ...
  theme_minimal() + 
  labs(
    title = 'Average BPRS Score as Function of Time and WZC',
    x = 'Year',
    y = 'BPRS Score'
  )
...
ggplot(dat, aes(x = as.factor(job), y = age, fill = as.factor(job))) +
  ...
  scale_fill_manual(values = c("#4575b4", "#d73027")) +
  theme_minimal(base_size = 14) +
  labs(title = "Age by Job", x = "Job", y = "Age") +
  theme(legend.position = "none")

ggplot(dat, aes(x = adl, y = age)) +
  geom_point(alpha = 0.6, color = "#4575b4") +
  geom_smooth(method = "lm", se = TRUE, color = "#d73027", linewidth = 1) +
  theme_minimal(base_size = 14) +
  labs(
    title = "Age at baseline as a Function of ADL",
    x = "ADL",
    y = "Age"
  )
...
Finally, given that the data are collected at regular time intervals, it is reasonable to assume an autoregressive correlation structure for the residual variance, capturing the expected correlation between adjacent time points (plot a semivariogram once the linear mixed model is fitted).
