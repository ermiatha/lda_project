...
# From Matteo ################################################################################################################################
This is not a multivariate model with unstructured covariance. If you use the argument random, it fits a mixed model (I was wrong about this during our last call). 

In a multivariate model you have: 

$Y = X\beta + \epsilon, \quad \epsilon \sim N(0, V)$

where you specify $V$ through

*   `correlation` = $\rightarrow$ off-diagonal structure of $V$
*   `weights` = $\rightarrow$ diagonal structure of $V$

Once you specify $V$, `gls()`:

*   constructs $V$
*   computes $V^{-1}$
*   plugs it into the GLS estimator

$\hat{\beta}_{GLS} = (X^\top V^{-1}X)^{-1}X^\top V^{-1}Y.$


Instead a Mixed model is:

$Y = X\beta + Zu + \epsilon$

*   The random effects $u$ are assumed to follow a normal distribution with a mean of 0 and a variance-covariance matrix $D$:
    $u \sim N(0, D)$

*   The residuals $\epsilon$ are assumed to follow a normal distribution with a mean of 0 and a variance-covariance matrix $R$:
    $\epsilon \sim N(0, R)$

*   The random effects $u$ and the residuals $\epsilon$ are assumed to be independent of each other.

`lme()` has two matrices:
*   D (random effects variance)
*   R (residual covariance)

And: 
$Var(Y) = Var(X\beta + Zu + \epsilon) = Var(Zu) + Var(\epsilon) = ZDZ^\top + R$

So in the multivariate model the V matrix is simply the Sigma matrix of the residuals. 
In the mixed model, the V matrix is the sum of the ZDZ' matrix (where D contains the variance/covariance of the random effect) + R (the matrix of the within-subject residuals). 

These two arguments, in gls and gls only

correlation = corSymm(form = ~ 1 | patid)
weights = varIdent(form = ~ 1 | time) #not time.num

give you an unstructured covariance. Specifying the random intercept give you a Compound Symmetry covariance, and the weights argument gives you heteroskedastic variance. 

Also, trial should not be included as a fixed effect. It is not a variable of interest, testing whether people from trial 16 have a statistically higher bprs value at baseline compared to people from trial 2 is not an interesting hypothesis, and it "wastes" a lot of degrees of freedom. 
...
I'm not sure what you mean here. getVarCov returns the V matrix, which is one per model, not one per patient. 
...
plot(m_full, resid(., type = "p") ~ fitted(.) | as.factor(time.num),
     abline = 0, ylab = "Fitted residuals")
ggsave("./../figures/q3_resid_vs_fitted_by_year.png", width = 8, height = 6, dpi = 300)
...
##########################################################################################################################################
I am not 100% sure what you mean here, but the OLS estimates will be different because OLS assumes i.i.d residuals, while gls does not 
...
numeric_vars <- dat_long[, c("age", "adl", "taupet.0", "abpet.0", "cdrsb.0")]
...
Model 1 already starts with a reduced covariance matrix. Starting with a unconstructed matrix is impractical and resource intensive, as the model would have to estimate 7(7+1)/2 = 28 parameters for the covariance matrix alone*

I decided to start with a heterogenous AR1 structure which assumes 7 different variances (one for each year), allowing to model heteroskedasticity, and assuming exponential decaying correlation between observations (hence estimating one single p parameter, bringing the total parameters to estimate to 8)

I started with a full model that includes all interaction effects, to test for the following, biologically plausible, hypothesis: 

- do people of different sex worsen faster? 
- do people of different age at baseline worsen faster? 
- do people with different taupet measurements at baseline worsen faster? 
- do people with different abpet measurements at baseline worsen faster? 
- do people with different cdrsb measurements at baseline worsen faster? 
- do people residing at home worsen faster? 
- do people with low daily activities at baseline worsen faster? 
- do people without a job worsen faster?


*after comparing two models, both with the same reduced mean structure, but one with an unconstructed covariance matrix and the other one with a heterogeneous AR1 covariance matrix, the latter has a much poorer fit than the first one. So we can't reduce the covariance structure to heterogenous AR1. Comparing the coefficients of the parameter estimes, I found that they vary quite a lot between the model with unstructured and the model with hete. AR1. 

I am afraid that reducing the mean structure starting from a hete.AR1 instead of unstr. could greatly affect the procedure of the reducing the mean. I don't have time now, but it might be worth looking into reducing the mean structure starting from an unconstructed covariance model (mind that it's quite time consuming). 
```{r}
library(nlme) 

# model with linear trends and Heterogeneous (weights=) AR1 covariance matrix

full_model <- bprs ~ time.num * (sex + age + taupet.0 + abpet.0 + cdrsb.0 + wzc + adl + job) 
...
```
We can see that time:wzc, time:abpet0, time:taupet0 and time:sex are not significant.

We might want to remove the non-significant interactions terms.

```{r}
full_model2 <- bprs ~ time.num * (age + cdrsb.0 + adl + job) + wzc + abpet.0 + taupet.0 + sex
...
```
Given that model 1 does not have a significantly better fit than model 2, we continue with the more parismonious model 2. 




Taupet0 is not significant, so we might want to remove it from the model.

```{r}
reduced_model3 <- bprs ~ time.num * (age + cdrsb.0 + adl + job) + wzc + abpet.0 + sex

m3 <- gls(reduced_model3, 
                         data = dat_long,
                         correlation = corAR1(form = ~ 1 | patid),
                         weights = varIdent(form = ~ 1 | time),
                         method = "ML",
...
anova(m3,m2)
```
Given that model 2 does not have a significantly better fit than model 3, we continue with the more parismonious model 3. 



The interaction effect of time:adl and time:cdrsb0 are significant, but the effect size is extremely small. We might want to test if we can remove them.



We can try to remove them from the model. 
```{r}
reduced_model4 <- bprs ~ time.num * (age + job) + wzc + abpet.0 + sex + cdrsb.0 + adl

m4 <- gls(reduced_model4, 
                         data = dat_long,
                         correlation = corAR1(form = ~ 1 | patid),
                         weights = varIdent(form = ~ 1 | time),
                         method = "ML",
...
anova(m4,m3)
```
Model 3 has a significantly better fit than model 4, so we have to keep the more complex model. 
...
Finally we can check if we can reduce even more the complexity of the covariance structure.
```{r}
m5 <- gls(full_model3, 
                         data = dat_long,
                         correlation = corSymm(form = ~ 1 | patid), 
...
It seems that a homoskedastic model is too simplistic for this data. 
