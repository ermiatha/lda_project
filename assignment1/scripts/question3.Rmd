...
# time * age * cdrsb0 * job
m2 <- gls(
    model = bprs ~ time.num * age * cdrsb.0 * job + wzc + trial + bmi, 
    weights = varIdent(form = ~ 1 | time.num),       # different variances per year
    method = "ML",
    data = dat_long,
    na.action = na.exclude
...
# time * (job + cdrsb.0)
m5 <- gls(
  model = bprs ~ time.num * (job + cdrsb.0) + age + wzc + trial + bmi, 
  weights = varIdent(form = ~ 1 | time.num),       # different variances per year
  method = "ML",
  data = dat_long,
  na.action = na.exclude
...
# time * cdrsb.0
m6 <- gls(
  model = bprs ~ time.num * cdrsb.0 +  age + trial + bmi + job + wzc, 
  weights = varIdent(form = ~ 1 | time.num),       # different variances per year
  method = "ML",
  data = dat_long,
  na.action = na.exclude
...
q3_tab_anovas_ia_latex <- tab_anovas %>% 
    kbl(format = "latex", 
                   booktabs = TRUE, 
                   caption = "Likelihood Ratio Tests used for model selection of main effects ",
                   label = "tab:q3_anova_main_effects"
...
```{r}
form_fixed <- bprs ~ time.num * age * cdrsb.0 * job + wzc + trial + bmi
```
...
```{r}
m_final <- m_final_unstr
# length(m_full$coefficients$fixed)
# noquote(names(m_full$coefficients$fixed))  # add: print as latex table
length(m_final$coefficients)
noquote(names(m_final$coefficients))  # add: print as latex table
