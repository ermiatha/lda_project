...
```{r}
ggplot(dat_long, aes(x = time, y = bprs, color = inkomen)) + 
  geom_jitter(alpha = 0.6) +
  scale_color_viridis_c(name = "Income") +
  theme_minimal() + 
  labs(
    title = 'Individual BPRS Scores by Time and Income',
    x = 'Year',
    y = 'BPRS Score'
  )

ggplot(dat_long_sample, aes(x = time, y = bprs, group = patid, color = inkomen)) +
  geom_line(alpha = 0.5, linewidth = 0.8) +
  scale_color_viridis_c(name = "Income") +
  theme_minimal() +
  labs(
    title = "Individual BPRS Profiles by Time and Income (Sample of 150 Patients)",
    x = "Year",
    y = "BPRS Score"
  )


ggplot(dat, aes(x = age_bin, y = inkomen, fill = age_bin)) +
  geom_boxplot(alpha = 0.7, color = "black") +
  scale_fill_manual(values = c("#b3cde0", "#005b96", "#03396c", "#011f4b")) +
  theme_minimal(base_size = 14) +
  labs(
    title = "Income at Baseline by Age Group",
    x = "Age Group",
    y = "Income"
  ) +
  theme(
